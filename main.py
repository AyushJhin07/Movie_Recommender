...
def load_models():
    ...
    """
    try:
        tfidf = joblib.load('TfidfVectorizer_1000.joblib')  # Path to TF-IDF Vectorizer
        knn = joblib.load('knn_model_low.joblib')  # Path to KNN Model
        combined_features = np.load('combined_features_normalized_low.npy')  # Path to Combined Features
...
def recommend_based_on_movies(selected_movies, df, combined_features, knn_model_low, top_n=10):
    ...
    distances, indices = knn_model_low.kneighbors(mean_vector_normalized, n_neighbors=n_neighbors)
